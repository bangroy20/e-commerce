...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['addToCart'])) {
        ...
        if ($numExistRows > 0) {
            ...
            $sql = "INSERT INTO `viewcart` (`cartTokoId`, `menuId`, `itemQuantity`, `userId`, `addedDate`) VALUES ('$cartTokoId', '$itemId', '1', '$userId', current_timestamp())";
    ...
    if (isset($_POST['checkout'])) {
        ...
        if (password_verify($password, $passRow['password'])) {
            ...
            if ($result) {
                ...
                echo '<script>alert("Terimakasih Telah Order, kode Ordermu ' . $orderId . '.");
                window.history.back(1);
                    </script>';
                exit();
            }
