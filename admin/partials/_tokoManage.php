...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['createToko'])) {
        $name = $_POST["name"];
        $desc = $_POST["desc"];

        $sql = "INSERT INTO `toko` (`tokoName`, `tokoDesc`, `tokoCreateDate`) VALUES ('$name', '$desc', current_timestamp())";
    ...
    if (isset($_POST['updateToko'])) {
        ...
        $sql = "UPDATE `tokos` SET `tokoName`='$catName', `tokoDesc`='$catDesc' WHERE `tokoId`='$catId'";
        $result = mysqli_query($conn, $sql);
        if ($result) {
